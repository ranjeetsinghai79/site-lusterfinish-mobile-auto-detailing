...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
        ...
        <tr>
          <td style="background:#111827;border:1px solid rgba(255,255,255,0.08);border-top:none;padding:32px 40px;">
            <p style="margin:0 0 18px;color:rgba(255,255,255,0.8);font-size:16px;line-height:1.7;">
              Hi — I came across <strong style="color:#fff;">${businessName}</strong> and noticed you don't have a website yet.
            </p>
            <p style="margin:0 0 18px;color:rgba(255,255,255,0.65);font-size:15px;line-height:1.7;">
              We use AI to build premium ${nicheLabel.toLowerCase()} websites overnight — cinematic design, lead capture forms, and an AI receptionist that answers every call 24/7 so you never miss a customer.
            </p>
            ${serviceList ? `
            ...
            <table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:8px;">
              ${[
                ['🌐', 'AI Website', '$299 one-time — yours to keep'],
                ['🤖', 'AI Receptionist', 'Answers every call 24/7 — $97/mo'],
                ['📈', 'AI Growth', 'Google reviews + monthly reports — $49/mo'],
              ].map(([icon, title, desc]) => `
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: fromEmail, to, subject: `Quick question about ${businessName}`, html }),
  })
  return res.ok
}
